...
- **Discord Developer Account** - Bot作成とOAuth2設定用
- **Riot Developer Account** - サモナー情報取得用（オプション）
...
サモナー名からMMRを自動計算する機能を使用する場合：

### 7.1 Riot Developer Accountの作成

1. [Riot Developer Portal](https://developer.riotgames.com/)にアクセス
2. Riotアカウントでログイン
3. Dashboard > My Appsから新しいアプリケーションを作成

### 7.2 API Keyの取得

1. Development API Key（24時間有効）を取得
2. 本番環境では Production API Key の申請が必要
3. APIキーをメモ
